...
void PrintTreeToFile(TreeNode* node, FILE* outFile, int sh = 0) {
    const int NSH = 3;

    for (int i = 0; i < sh; i++) fprintf(outFile, " ");

    fprintf(outFile, "[%s]", NodeKindStr[node->node_kind]);

    if (node->node_kind == OPER_NODE) fprintf(outFile, "[%s]", TokenTypeStr[node->oper]);
    else if (node->node_kind == NUM_NODE) fprintf(outFile, "[%d]", node->num);
    else if (node->node_kind == ID_NODE || node->node_kind == READ_NODE || node->node_kind == ASSIGN_NODE)
        fprintf(outFile, "[%s]", node->id);

    if (node->expr_data_type != VOID) fprintf(outFile, "[%s]", ExprDataTypeStr[node->expr_data_type]);
